...
class DataManager {
    ...
    populateDemoDataIfEmpty() {
        if (this.data.projects.length === 0) {
            ...
            const demoProject = {
                ...
                budget: 1250000,
                progress: 45,
                description: 'A 12-unit modern luxury residential complex with sustainable energy systems.',
            ...
            const demoClient = {
                ...
                email: 'contact@horizondev.com',
                phone: '(555) 012-3456',
                address: '789 Skyline Blvd, Suite 400',
                industry_id: 1
            };
            this.data.clients.push(demoClient);
            demoProject.client_id = demoClient.id;
            demoProject.client_name = demoClient.name;

            // Add a demo estimate
            this.data.estimates.push({
                id: 301,
                project_id: 101,
    ...
    getMaterials(industryId = null) {
        if (!industryId) return this.data.materials;
        return this.data.materials.filter(m => m.industry_id == industryId);
    }

    getLabor(industryId = null) {
        if (!industryId) return this.data.labor;
        return this.data.labor.filter(l => l.industry_id == industryId);
    }
    ...
    getSchedules(projectId = null) {
        if (!projectId) return this.data.schedules;
        return this.data.schedules.filter(s => s.project_id == projectId);
    }

    getDailyLogs(projectId = null) {
        if (!projectId) return this.data.daily_logs;
        return this.data.daily_logs.filter(l => l.project_id == projectId);
    }
