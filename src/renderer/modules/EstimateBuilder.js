...
class EstimateBuilder {
    ...
    async loadItemsForModal() {
        ...
        try {
            // Load materials
            const materials = window.dataManager ? window.dataManager.getMaterials(industryId) : [];
            ...
            if (materialsBody) {
                materialsBody.innerHTML = materials.map(material => `
                    <tr>
                        ...
                        <td>${material.unit_type || material.unit || 'unit'}</td>
                        <td>
            ...
            // Load labor
            const labor = window.dataManager ? window.dataManager.getLabor(industryId) : [];
            const laborBody = document.getElementById('laborTableBody');
            
            if (laborBody) {
                laborBody.innerHTML = labor.map(laborItem => `
                    <tr>
                        <td><strong>${laborItem.task_name || laborItem.name}</strong></td>
                        <td>${laborItem.description || 'N/A'}</td>
                        <td>$${(parseFloat(laborItem.hourly_rate || laborItem.unit_cost || 0)).toFixed(2)}</td>
    ...
    async saveEstimate() {
        ...
        try {
            let estimateId;
            if (window.dataManager) {
                const saved = await window.dataManager.saveData('estimates', estimateData);
                estimateId = saved.id;
            } else {
                estimateId = Date.now();
                estimateData.id = estimateId;
                const estimates = JSON.parse(localStorage.getItem('constructpro_estimates') || '[]');
                estimates.push(estimateData);
                localStorage.setItem('constructpro_estimates', JSON.stringify(estimates));
            }
            
            // Show success message
            if (window.app) {
                window.app.showAlert('success', `Estimate saved successfully!`);
            }
